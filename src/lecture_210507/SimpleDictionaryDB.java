...
public class SimpleDictionaryDB extends JPanel implements ActionListener {
    ...
    // private final static DICT_FILE_NAME = "resource/";
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource() == searchBtn) {
            ...
            if (key.trim().length() == 0 || value ==  null) {
                JOptionPane.showMessageDialog(this, "Can't Found word", "Found Failed", JOptionPane.ERROR_MESSAGE);
            } else {
            ...
            if (key.trim().length() > 0 && value.trim().length() > 0) {
                dict.put(key, value);
                addToDB(key,value);
                JOptionPane.showMessageDialog(this, "Success Add to word", "Add Success", JOptionPane.INFORMATION_MESSAGE);
            }
        } repaint();
    }
    ...
    private void addToDB(String key, String value) {
        try (Connection con = DriverManager.getConnection(dbURL, userName, userPassword)){
            String sql = "insert into simple_dict(keyword,values) values (?,?)";
